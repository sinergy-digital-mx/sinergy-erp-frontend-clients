...
var DataMapperService = class _DataMapperService {
  ...
  mapUser(backendUser) {
    return __spreadValues({
      ...
      has_open_global_cut: backendUser.has_open_global_cut ?? backendUser.has_open_global_corte ?? backendUser.hasOpenGlobalCut ?? false,
      ...
      has_all_branches_access: backendUser.has_all_branches_access ?? backendUser.hasAllBranchesAccess
    }, backendUser);
  }
  /**
...
var UserService = class _UserService {
   ...
   */
  getUserRoles(userId) {
    return this.http.get(`${this.api}/tenant/users/${userId}/roles`).pipe(map((backendRoles) => this.dataMapper.mapRoles(backendRoles)));
  }
  /**
  ...
  getUserBranch(userId) {
    return this.http.get(`${this.api}/tenant/users/${userId}/branch`).pipe(map((response) => response?.billing_branch_id ?? null));
  }
  updateUserBranch(userId, billingBranchId) {
    return this.http.put(`${this.api}/tenant/users/${userId}/branch`, {
      billing_branch_id: billingBranchId
    }).pipe(tap(() => this.clearCache()), shareReplay(1));
  }
  /**
...
//# sourceMappingURL=chunk-PJSUQ4Q5.js.map
