...
// src/app/features/pos/utils/pos-discount.util.ts
function previewLineDiscount(unitPrice, qty, discount) {
...
function formatApplicableDiscountLabel(discount) {
  if (discount.discount_type === "percentage") {
...
function isDiscountApiError(message) {
...
var POSService = class _POSService {
  ...
  addItem(item) {
    ...
    const existingItemIndex = currentCart.items.findIndex((i) => i.product_id === item.product_id && i.product_uom_id === item.product_uom_id && (i.product_discount_id ?? null) === (item.product_discount_id ?? null));
    let updatedItems;
  ...
  recalculateItem(item) {
    const preview = previewLineDiscount(item.unit_price, item.quantity, item.selected_discount);
...
//# sourceMappingURL=chunk-MK3LIK2T.js.map
