...
var SelectComponent = class _SelectComponent {
  ...
  static \u0275cmp = /* @__PURE__ */ \u0275\u0275defineComponent({ type: _SelectComponent, selectors: [["app-select"]], inputs: { config: "config", disable: "disable", has_error: "has_error", label: "label" }, outputs: { changeOption: "changeOption", refresh: "refresh" }, features: [\u0275\u0275ProvidersFeature([
    {
      provide: NG_VALUE_ACCESSOR,
      useExisting: forwardRef(() => _SelectComponent),
      multi: true
    }
  ]), \u0275\u0275NgOnChangesFeature], decls: 10, vars: 18, consts: [[1, "container"], [1, "block", "text-sm", "font-medium", "text-gray-700", "mb-1"], [1, "select_container"], [1, "filter__control", 3, "blur", "id", "formControl", "disabled"], ["disabled", "", "selected", "", 3, "ngValue"], [3, "ngValue"], ["disabled", "", "selected", "", "hidden", "", 3, "ngValue"], [1, "ti", "ti-refresh"], ["role", "alert", 1, "mt-1", "text-sm", "text-red-600", 3, "id"], [1, "text-red-500"], [3, "ngValue", "disabled"], [1, "ti", "ti-refresh", 3, "click"]], template: function SelectComponent_Template(rf, ctx) {
    if (rf & 1) {
      \u0275\u0275elementStart(0, "div", 0);
      \u0275\u0275conditionalCreate(1, SelectComponent_Conditional_1_Template, 3, 3, "label", 1);
      \u0275\u0275elementStart(2, "div", 2)(3, "select", 3);
      \u0275\u0275listener("blur", function SelectComponent_Template_select_blur_3_listener() {
...
(() => {
  (typeof ngDevMode === "undefined" || ngDevMode) && setClassMetadata(SelectComponent, [{
    type: Component,
    args: [{ selector: "app-select", standalone: true, imports: [ReactiveFormsModule, CommonModule], providers: [
      {
        provide: NG_VALUE_ACCESSOR,
        useExisting: forwardRef(() => SelectComponent),
        multi: true
      }
    ], template: `<div class="container" [class.error]="showError">
  @if (config.label || label) {
    <label [attr.for]="selectId" class="block text-sm font-medium text-gray-700 mb-1">
      {{label ?? config.label}}@if (showError) {
...
<div class="select_container">
  <select
    ...
    [formControl]="config.form_control ? config.form_control : select"
    [style.min-height]="config.height ? config.height + 'px' : '100%'"
    ...
    >
    @if (config.loading) {
      <option [ngValue]="null" disabled selected>Cargando...</option>
    }
    @if (!config.loading && config.all) {
      <option [ngValue]="null">{{ config.all_message ? config.all_message : 'All' }}</option>
    }
    @if (!config.loading && !config.all) {
    ...
    @if (!config.loading) {
      @for (item of config.data; track item) {
        <option [ngValue]="item[config.value]" [disabled]="item[config.disabled_option]">
          {{ item[config.option] }}
        </option>
      }
    }
  ...
  @if (config.error) {
    <i class="ti ti-refresh" (click)="getRefresh()"></i>
  }
</div>
...
(() => {
  (typeof ngDevMode === "undefined" || ngDevMode) && \u0275setClassDebugInfo(SelectComponent, { className: "SelectComponent", filePath: "src/app/core/components/select/select.component.ts", lineNumber: 35 });
})();

export {
  SelectComponent
};
//# sourceMappingURL=chunk-KW3QIBBO.js.map
